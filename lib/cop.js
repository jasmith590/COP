...
exports.gatherInputs = function (filename, inputFormat) {
    ...
    if(inputs.length == 1 && !fs.existsSync(filename)) {
        console.error("File does not exist '" + filename + "'");
        process.exit(1);
    }

    // Check if directory
    if(inputs.length == 1 && fs.statSync(fs.realpathSync(filename)).isDirectory()){
        inputs = this.fileList(fs.realpathSync(filename), this.isValidFormat.bind(self));
    }
    ...
    if(inputs.length > 1){
        ...
        return this.load(fs.realpathSync(filename), inputFormat);
    }
...
exports.fileList = function(dir, testCase) {
    return fs.readdirSync(dir).reduce((list, file) => {
        if(typeof testCase === "function" && !testCase(handy.getFileExtension(file))) {
            return list;
        }
