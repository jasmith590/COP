...
exports.parse = function(shellvars) {
    ...
    shellvars.split(/\n+/).forEach((assignment) => {
        ...
        if (assignment.length <= 0 || assignment.indexOf("#") >= 0) return;
...
exports.normalizeString = function(value) {
    ...
    // escape double quotes in value (only if not already escaped!)
 ...
 * - Parses ["a", "b"] => ( "a" "b")
 * - Parses [{a: b}, {c: d}] => {0: {a: b}, 1: {c: d}}  (use cop recursibely)
 *
 * @param obj
 * @returns {string}
 */
exports.parseArray = function(obj) {
    return '( ' + obj.map(this.normalizeString, this).join(" ") + ' )';
};
